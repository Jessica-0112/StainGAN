...
def collect_patient_keys_and_summary(src_lmdb: Path) -> Dict[str, dict]:
    env = lmdb.open(
        str(src_lmdb),
        readonly=True,
        lock=False,
...
def make_fixed_test_5fold_split(
    patients: List[str],
    ...
    for fold_idx in range(n_folds):
        ...
        fold_dict = {
            ...
            "counts": {
                ...
                "train_slides": sum(patient_summary[p]["num_slides"] for p in train_patients),
                "val_slides": sum(patient_summary[p]["num_slides"] for p in val_patients),
                "test_slides": sum(patient_summary[p]["num_slides"] for p in test_patients),
            },
        }

        if store_keys:
            fold_dict["train_keys"] = train_keys
            fold_dict["val_keys"] = val_keys
            fold_dict["test_keys"] = test_keys

        folds[f"fold{fold_idx}"] = fold_dict

    # 為了避免 JSON 太大，patient_summary 裡可以保留統計，不一定要保留每個 patient 的 keys
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create patient-level fixed-test + 5-fold split JSON with fold keys."
    )
    ...
    parser.add_argument(
        "--no-store-keys",
        action="store_true",
        help="Only store patient split, not key lists. Not recommended for faster StainGAN loading.",
    )
    ...
    patient_summary = collect_patient_keys_and_summary(src_lmdb)
    patients = sorted(patient_summary.keys())

    split_info = make_fixed_test_5fold_split(
        patients=patients,
        patient_summary=patient_summary,
        test_n=args.test_n,
        n_folds=args.n_folds,
        seed=args.seed,
        store_keys=not args.no_store_keys,
    )
